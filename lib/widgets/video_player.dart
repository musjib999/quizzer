...
class QuizzerMediaSize {
  final int width;
  final int height;

  static const zero = const QuizzerMediaSize(0, 0);

  const QuizzerMediaSize(int width, int height)
      : this.width = width,
        this.height = height;
}

class QuizzerPlayer extends StatefulWidget {
...
class _VlcPlayerState extends State<QuizzerPlayer>
...
class VlcPlayerController {
  ...
  QuizzerMediaSize get size => _size != null ? _size : QuizzerMediaSize.zero;
  QuizzerMediaSize _size;
      ...
      switch (event['name']) {
        case 'playing':
          if (event['width'] != null && event['height'] != null)
            _size = new QuizzerMediaSize(event['width'], event['height']);
